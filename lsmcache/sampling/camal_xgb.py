    ...
    def run(self):
        ...
        for workload in workloads:
            ...
            for size_ratio in T_list:
                ...
                df.append(row) # 实际运行结果加入采样集df
                pd.DataFrame(df).to_csv(self.config["samples_path"]["camal_xgb_ckpt"])
                step += 1
                self.logger.info(f"Used {time.time()-start_time}s\n")

            # iter model
            regr = iter_model(df, "level", E, M, N) # E(Bytes)
            ...
            min_err = 1e9
            for ratio in [0.6, 0.8]: # 0.6/0.8
                buffer = ratio * (M - h0 * N) / 8
        ...
        pd.DataFrame(df).to_csv(self.config["samples_path"]["camal_xgb_final"])
