...
class RocksDB(object):
    ...
    def __init__(self, config):
        ...
        # ✅ 增加关于I/O Cost统计的解析
        self.io_cost_prog = re.compile(
            r"\[[0-9:.]+\]\[info\] \(write_io_kb_per_op, read_io_kb_per_op, total_io_kb_per_op\) : "
            r"\(([0-9.]+), ([0-9.]+), ([0-9.]+)\)"
        )
    ...
    def _log_run_params(self, db_dir, num_z0, num_z1, num_q, num_w, queries, 
        ...
        self.logger.info(f"  Mbuf (initial write buffer)  : {self.initial_mbuf} bytes ({self.initial_mbuf / 1024 / 1024:.2f} MB)")
    ...
    def _get_default_results(self):
            ...
            return {
                ...
                "cache_miss": 0, 
                "init_time": 0,
                "write_io_kb_per_op": 0.0,
                "read_io_kb_per_op": 0.0,
                "total_io_kb_per_op": 0.0,
            }
    ...
    def _parse_results(self, proc_results):
        ...
        parse_configs = [
            # ✅ 增加对于Memory Tuner中的I/O成本的解析
            (self.io_cost_prog,
             ["write_io_kb_per_op", "read_io_kb_per_op", "total_io_kb_per_op"],
             float,
             "io_cost (write_io_kb_per_op, read_io_kb_per_op, total_io_kb_per_op)"),
    ...
    def run(
        ...
        self.T, self.N, self.E, self.M, self.h, self.initial_mbuf = int(T), int(N), int(E), int(M), int(h), int(initial_mbuf)
        # 调用日志函数进行格式化输出
